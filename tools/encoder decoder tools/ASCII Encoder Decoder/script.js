...
function encodeBtn() {
    ...
    const asciiEncoded = encodeToASCII(inputText);
    document.getElementById("outputText").value = asciiEncoded;
}
...
function decodeFromASCII(asciiStr) {
    return asciiStr.split(' ').map(code => String.fromCharCode(code)).join('');
}

function decodeBtn() {
    const inputText = document.getElementById("inputText").value;
    const decodedText = decodeFromASCII(inputText);
