...
const reverseNatoAlphabet = Object.fromEntries(
    Object.entries(natoAlphabet).map(([key, value]) => [value.toLowerCase(), key])
);

function encodeBtn() {
    const inputText = document.getElementById('inputText').value.toUpperCase();
    const outputText = inputText.split('').map(char => natoAlphabet[char] || char).join(' ');
...
function decodeBtn() {
    ...
    const outputText = inputText.split(' ').map(word => reverseNatoAlphabet[word] || word).join('');
    document.getElementById('outputText').value = outputText;
}
