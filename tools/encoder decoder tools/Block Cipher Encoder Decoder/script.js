...
function randomKeyBtn() {
    var randomKey = generateRandomKey(16);
...
function encodeBtn() {
    ...
    var outputText = CryptoJS.AES.encrypt(inputText, key).toString();
...
function decodeBtn() {
    var inputText = document.getElementById('inputText').value;
    var key = document.getElementById('secretKey').value;
    const outputText = CryptoJS.AES.decrypt(inputText, key).toString(CryptoJS.enc.Utf8);
    if (outputText) {
        document.getElementById('outputText').value = outputText;
    } else {
