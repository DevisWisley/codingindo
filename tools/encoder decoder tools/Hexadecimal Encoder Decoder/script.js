...
function encodeBtn() {
    ...
    const isHex = /^[0-9a-fA-F ]+$/.test(inputText.trim());
    if (isHex) {
        document.getElementById('outputText').value = inputText;
    } else {
        const hexEncoded = encodeToHex(inputText);
        document.getElementById('outputText').value = hexEncoded;
    }
}
...
function decodeFromHex(hex) {
    ...
    hexArray.forEach(function (value) {
        str += String.fromCharCode(parseInt(value, 16));
    });
    return str;
}
...
function decodeBtn() {
    const inputText = document.getElementById('inputText').value;
    const decodedText = decodeFromHex(inputText);
