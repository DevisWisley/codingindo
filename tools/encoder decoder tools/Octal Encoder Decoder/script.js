...
function encodeToOctal(str) {
    return str.split('').map(char => char.charCodeAt(0).toString(8)).join(' ');
}

function encodeBtn() {
    const inputText = document.getElementById('inputText').value;
    const encodedArrays = encodeToOctal(inputText);
    document.getElementById('outputText').value = encodedArrays;
}
...
function decodeFromOctal(octal) {
    return octal.split(' ').map(code => String.fromCharCode(parseInt(code, 8))).join('');
}

function decodeBtn() {
    const encodedArrays = document.getElementById('inputText').value;
    const decodedArrays = decodeFromOctal(encodedArrays);
    document.getElementById('outputText').value = decodedArrays;
}
