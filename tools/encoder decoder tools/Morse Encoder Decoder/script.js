...
function encodeToMorse(str) {
    return str.toUpperCase().split('').map(char => morseCode[char] || '').join(' ');
}

function encodeBtn() {
    const inputText = document.getElementById('inputText').value.trim();
    const outputText = encodeToMorse(inputText);
...
function decodeFromMorse(str) {
    const reversedMorseCode = Object.fromEntries(Object.entries(morseCode).map(([key, value]) => [value, key]));
    return str.split(' ').map(code => reversedMorseCode[code] || '').join('');
}

function decodeBtn() {
    const encodedArrays = document.getElementById('inputText').value.trim();
    const decodedArrays = decodeFromMorse(encodedArrays);
