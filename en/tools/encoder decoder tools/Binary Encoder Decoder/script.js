...
function encodeBtn() {
    const binary = inputText.value.split('').map(char => char.charCodeAt(0).toString(2).padStart(8, '0')).join(' ');
    outputText.value = binary;
}
...
function decodeBtn() {
    const binary = inputText.value.split(' ').map(byte => String.fromCharCode(parseInt(byte, 2))).join('');
    outputText.value = binary;
}
