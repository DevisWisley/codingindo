...
document.getElementById('colorPicker').addEventListener('input', function() {
    ...
    document.getElementById('inputText').value = colorValue;
});
...
function encodeBtn() {
    ...
    const encodedColor = hexToRgb(inputText);
    document.getElementById('outputText').value = encodedColor;
}
...
function decodeBtn() {
    const inputText = document.getElementById('inputText').value;
    const match = inputText.match(/rgb\((\d+), (\d+), (\d+)\)/);
    if (match) {
        const [_, r, g, b] = match;
        document.getElementById('outputText').value = rgbToHex(parseInt(r), parseInt(g), parseInt(b));
    } else {
        alert('Please convert a color first!');
    }
